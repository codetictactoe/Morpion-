...
class TIC_TAC_TOE_AI:
    ...
    def decorating(self):# Basic Set-up
        Label(self.make_canvas, text="Tic-Tac-Toe AI", bg="#141414", fg="#ff002b", font=("Lato", 25, "bold")).place(x=110, y=10)
    ...
    def __sign_insert(self, btn_indicator, sign_is="O"):# Button sign Insert
        if sign_is == "O":
            self.activate_btn[btn_indicator - 1].config(text=sign_is, state=DISABLED, disabledforeground="#00c3ff")
        else:
    ...
    def __machine_play(self):# Machine Control
        ...
        if self.chance_counter == 1:
            ...
            if human_input%2 == 0:
                self.technique = 1
                self.activate_btn[5 - 1].config(text="O")
                self.sign_store[5] = "O"
                self.prob.append(1)
            ...
            if self.technique == 1:
                if self.prob[0] != human_input:
                    ...
                    if self.technique == 1:
                        try:
                            try:
                                if self.sign_store[self.prob[0]+1] == "X":
                                    pass
                            except:
                                if self.sign_store[self.prob[0]+1+6] == "X":        
                                    pass
                    ...
                    if self.machine_cover[0] - self.machine_cover[1] == 6:
                        try:
                            if self.sign_store[self.machine_cover[1]+3] == "X":
                                self.prob.clear()
                        ...
                        try:
                            if self.sign_store[self.machine_cover[1]+1] == "X":
                                self.prob.clear()
